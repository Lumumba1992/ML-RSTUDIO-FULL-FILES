...
```{r}
sjt.xtab(mydata$Type, mydata$Sex, show.col.prc = TRUE, show.row.prc = TRUE)
```
...
```{r}
p <- predict(model1, mydata)
head(p)
```

```{r}
confusionMatrix(p, reference=mydata$Type.f, positive="CLA")
```
...
## confusion Matrix
```{r}
confusionMatrix(predict(model2, mydata),
                reference = mydata$Type.f, positive = "CLA")
```
...
### Alternative Consusion Matrix
```{r}
confusionMatrix(predict(mymodel, training), 
                reference = training$NSP, positive = "1")
```
...
### Build the Classification and Regression Tree (CART) Model
...
model.cart <- train( NSP ~ ., data=train_data, 
  method = "rpart",
...
### Confusion Matrix
```{r}
confusionMatrix(predict(model.cart, train_data), 
                reference=train_data$NSP, positive="Normal")
```

### F1 Score
```{r}
predictions <- predict(model.cart, train_data, type = "raw")
confusion.matrix <- table(train_data$NSP, predictions)
precision <- (confusion.matrix[1, 1] / sum(confusion.matrix[, 1])) * 100
recall <- (confusion.matrix[1, 1] / sum(confusion.matrix[1, ])) * 100
...
```{r}
plot(varImp(model.cart), main = "Variable Importance for CART Model")
```
...
The test data has not been seen by the random forest model and the accuracy has come down to 93.34%. The 95% CI is still good, ranging between 91% to 95%. There is slightly higher misclassification from the test data as compared to the train data. Besides, sensitivity for predicting class 1 is still higher as compared to class 2 and 3.
...
## Additional Machine Learning Algorithm (CART MODEL)
```{r}
mydata4 <- read.csv("Cardiotocographic.csv", header = TRUE)
...
### Decision Tree for Hear Failure
```{r}
...
### Plot the Tree
This R code performs two operations on the dataset "iris":

It assigns the dataset "iris" to the variable "flowers_data".
It filters out rows where the species is "virginica" and reassigns the filtered dataset back to "flowers_data".
So, after executing this code, "flowers_data" will contain only the rows where the species is not "virginica".
```{r fig.width=12}
flowers_data <- iris
...
data <- data.frame(Actual_MEDV = data_pred$medv, Predicted_MEDV = data_pred$pred)

# Create line plot
ggplot(data, aes(x = 1:nrow(data))) +
  geom_line(aes(y = Actual_MEDV, color = "Actual MEDV")) +
  geom_line(aes(y = Predicted_MEDV, color = "Predicted MEDV")) +
  scale_color_manual(name = "Variable", values = c("Actual MEDV" = "blue", "Predicted MEDV" = "red")) +
  labs(x = "Time Axis", y = "MEDV", title = "Actual vs Predicted MEDV") +
  theme_economist()
...
### Compare the Traditional Linear Model to the KNN Model
```{r}
library(sjPlot)
lmodel <- lm(medv~.,data = training,)
tab_model(lmodel,
          show.se = TRUE, #show.std = FALSE,
          show.df = TRUE,
          show.stat = TRUE,
          #, auto.label = FALSE,
          digits = 3,digits.p = 3,digits.rsq = 3,digits.re = 2)
```

The traditional linear model performed poorly as compared to the kNN. The linear model has an R-square of 0.777 and an adjusted R-square of 0.769, while kNN had an R-square of 0.8059345, which is higher than that of traditional linear model. 
...
```{r}
set.seed(123)
t.model <- tune.svm(Species~., data = iris, epsilon = seq(0,1,0.1), cost=2^(2:9)
                    )
```
...
```{r}
set.seed(123)
t.model1 <- tune.svm(Species~., data = iris, epsilon = seq(0,1,0.1), cost=2^(2:7)
                     )
```
