...
set.seed(1234)
t.model <- tune.svm(Species~., data = iris, gamma = 2^(-1:1), cost = 2^(2:4))
plot(t.model)
```
...
data <- iris
set.seed(123)
t.model <- tune.svm(Species~., data = iris,  cost=2^(2:9), epsilon = seq(0,1,0.1))
plot(t.model)
```
...
The sampling method is 10 fold cross validation with the best parameters as epsilon = 0 and cost = 4. Using these results we can choose our best model. 
```{r}
...
### View the Summary of the Final Best Model
```{r}
mymodel$cost
mymodel$epsilon
```
